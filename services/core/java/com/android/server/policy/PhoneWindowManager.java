...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void beginLayoutLw(DisplayFrames displayFrames, int uiMode) {
        ...
        if (displayFrames.mDisplayId == DEFAULT_DISPLAY) {
            ...
            // if (! mNavigationBar && mUseGestureButton && mGestureButton != null) {
        ...
        if (mDeviceKeyHandler != null) {
            try {
                ...
                if (!interactive && eventLaunchActivity != null) {
                    ...
                    wakeUp(event.getEventTime(), mAllowTheaterModeWakeFromKey, "android.policy:KEY");
                    AquaCafUtils.launchKeyguardDismissIntent(mContext, UserHandle.CURRENT, eventLaunchActivity);
                    result &= ~ACTION_PASS_TO_USER;
                    return result;
                }
    ...
    public void sendCustomAction(Intent intent) {
        ...
        if (action != null) {
            if (ActionHandler.INTENT_SHOW_POWER_MENU.equals(action)) {
                showGlobalActions();
            } else if (AquaUtils.INTENT_SCREENSHOT.equals(action) ||
                    ActionHandler.INTENT_SCREENSHOT.equals(action)) {
                mContext.enforceCallingOrSelfPermission(ACCESS_SURFACE_FLINGER,
                ...
                mHandler.post(mScreenshotRunnable);
                    ...
                    ActionHandler.INTENT_REGION_SCREENSHOT.equals(action)) {
                mContext.enforceCallingOrSelfPermission(ACCESS_SURFACE_FLINGER,
                        TAG + "sendCustomAction permission denied");
                mHandler.removeCallbacks(mScreenshotRunnable);
