...
public class QuickStepController implements GestureHelper {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (mNavigationBarView.isDt2s() && isDoubleTapPending) {
                    ...
                    boolean intentionalDoubleTap = deltaX * deltaX + deltaY * deltaY < sDoubleTapSquare;
                    if (intentionalDoubleTap) {
                        mNavigationBarView.performHapticFeedback(HapticFeedbackConstants.VIRTUAL_KEY);
                        AquaUtils.switchScreenOff(mContext);
                    }
                } else {
    ...
    private Runnable mDoubleTapCancelTimeout = new Runnable() {
        ...
        public void run() {
            ...
            mHandler.removeCallbacksAndMessages(null);
            mNavigationBarView.performHapticFeedback(HapticFeedbackConstants.VIRTUAL_KEY);
            AquaUtils.sendKeycode(KeyEvent.KEYCODE_HOME, mHandler);
        }
    };
    ...
    private void moveKbCursor(boolean right, boolean firstTrigger) {
        ...
        AquaUtils.moveKbCursor(KeyEvent.ACTION_UP, right);
        AquaUtils.moveKbCursor(KeyEvent.ACTION_DOWN, right);
